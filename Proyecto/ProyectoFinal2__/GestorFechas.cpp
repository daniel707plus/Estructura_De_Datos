...
// ============================================================
//  GESTOR DE FECHAS - Sistema de Matriculacion Vehicular Ecuador
//  Reglas:
//   - No se asignan turnos en sabados ni domingos.
//   - No se asignan turnos en feriados nacionales de Ecuador.
//   - La fecha debe estar entre hoy y exactamente 1 anio adelante.
// ============================================================
...
std::string GestorFechas::ajustarFechaLaborable(std::tm* fecha) {
    ...
    // Lambda: nombre del feriado fijo si la fecha lo es (sin traslado, fecha exacta)
    auto nombreFeriadoFijo = [&normalizar](auto& self, int dEval, int mEval, int aEval) -> std::string {
        // Pares: (dia, mes, nombre)
        // Usamos lambda recursiva para recorrer sin arreglos
        ...
        if (choca(1,  1))  return "Anio Nuevo (1 Ene)";
        if (choca(1,  5))  return "Dia del Trabajo (1 May)";
        if (choca(24, 5))  return "Batalla de Pichincha (24 May)";
        if (choca(10, 8))  return "Primer Grito de Independencia (10 Ago)";
        if (choca(9,  10)) return "Independencia de Guayaquil (9 Oct)";
        if (choca(2,  11)) return "Dia de los Difuntos (2 Nov)";
        if (choca(3,  11)) return "Independencia de Cuenca (3 Nov)";
        if (choca(25, 12)) return "Navidad (25 Dic)";
        return "";
    };
    ...
    while (ajustado) {
        ...
        // Feriado (con traslado de ley) -> empujar 1 dia
        if (esFeriadoFijo(esFeriadoFijo, fecha->tm_mday, fecha->tm_mon + 1,
                          fecha->tm_year + 1900, 1, 1)   ||
            esFeriadoFijo(esFeriadoFijo, fecha->tm_mday, fecha->tm_mon + 1,
                          fecha->tm_year + 1900, 1, 5)   ||
            esFeriadoFijo(esFeriadoFijo, fecha->tm_mday, fecha->tm_mon + 1,
                          fecha->tm_year + 1900, 24, 5)  ||
            esFeriadoFijo(esFeriadoFijo, fecha->tm_mday, fecha->tm_mon + 1,
                          fecha->tm_year + 1900, 10, 8)  ||
            esFeriadoFijo(esFeriadoFijo, fecha->tm_mday, fecha->tm_mon + 1,
                          fecha->tm_year + 1900, 9, 10)  ||
            esFeriadoFijo(esFeriadoFijo, fecha->tm_mday, fecha->tm_mon + 1,
                          fecha->tm_year + 1900, 2, 11)  ||
            esFeriadoFijo(esFeriadoFijo, fecha->tm_mday, fecha->tm_mon + 1,
                          fecha->tm_year + 1900, 3, 11)  ||
            esFeriadoFijo(esFeriadoFijo, fecha->tm_mday, fecha->tm_mon + 1,
                          fecha->tm_year + 1900, 25, 12) ||
            esFeriadoMovil(esFeriadoMovil, fecha->tm_mday, fecha->tm_mon + 1,
                           fecha->tm_year + 1900)) {
            fecha->tm_mday += 1;
            normalizar(fecha);
            ajustado = true;
