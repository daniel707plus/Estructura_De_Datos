...
bool ListaDoble::hayCruceDeHorarios(int dia, int mes, int anio, int hIniNuevo, int mIniNuevo, int hFinNuevo, int mFinNuevo) const {
    // Convertimos a minutos desde la medianoche para comparar fácilmente
    int totalIniNuevo = (hIniNuevo * 60) + mIniNuevo;
    int totalFinNuevo = (hFinNuevo * 60) + mFinNuevo;

    Nodo* actual = cabeza;
    while (actual != nullptr) {
        std::tm f = actual->getReserva()->getFechaAsignada();
        
        // 1. Solo comparamos si es exactamente el MISMO DÍA
        if (f.tm_mday == dia && f.tm_mon == (mes - 1) && f.tm_year == (anio - 1900)) {
            
            int totalIniExistente = (actual->getReserva()->getHInicio() * 60) + actual->getReserva()->getMInicio();
            int totalFinExistente = (actual->getReserva()->getHFin() * 60) + actual->getReserva()->getMFin();
            
            // 2. FÓRMULA DE CRUCE: Si el inicio del nuevo es menor al fin del existente, 
            // Y el fin del nuevo es mayor al inicio del existente, hay choque.
            if (totalIniNuevo < totalFinExistente && totalFinNuevo > totalIniExistente) {
